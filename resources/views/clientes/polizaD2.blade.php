...
<table style="font-size: 14px;">
  <tr>
    ...
    <th style="position: absolute;left: 12%; top: 21.5%"><b>{{$contrato->contrato->nombres}} {{$contrato->contrato->apellidos}}</b></th>
    <th></th>
  </tr>
  <tr>
    <td style="position: absolute;left: 14%;top: 24.3%;"><b>{{$direccion->calle}}</b></td>
    <th style="position: absolute;left: 77%;top: 24.3%"><b>{{$contrato->contrato->telefono}}</b></th>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <tr>
    ...
    <tr>
    <td style="position: absolute;left: 24%;top: 56%;"><b>{{money_format('%n', $pagos->costoservicio)}}</b></td>
    @if($contrato->plazo == '0.1')
      <td style="position: absolute;left: 52%;top: 56%;"><b>{{date("d/m/Y", strtotime($contrato->desde))}}</b></td>
    @else
    <td style="position: absolute;left: 52%;top: 56%;"><b>{{date("d/m/Y", strtotime($pagos->pagoinicial))}}</b></td>
    @endif
    <td style="position: absolute;left: 83%;top: 56%;"><b>{{$pagos->forma_pago}}</b></td>
  ...
  <tr>
    <td style="position: absolute;left: 14%;top: 90%;"><b>@if($contrato->plazo == '0.5')6 MESES @elseif($contrato->plazo == '0.1')1 MES @else {{$contrato->plazo}} AÑOS @endif</b></td>
    <td style="position: absolute;left: 45%;top: 90%;"><b>{{ date("d/m/Y", strtotime($contrato->desde))}}</b></td>
    <td style="position: absolute;left: 78%;top: 90%;"><b>{{ date("d/m/Y", strtotime($contrato->hasta))}}</b></td>
  </tr>
</table>
